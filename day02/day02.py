...
class Game:
    ...
    def is_possible(self, bag):
        return all(game_set.is_possible(bag) for game_set in self.game_sets)

    @property
    def minimum_bag(self):
        return GameSet(max(game_set.red_cubes for game_set in self.game_sets),
                       max(game_set.green_cubes for game_set in self.game_sets),
                       max(game_set.blue_cubes for game_set in self.game_sets))

    @property
    def power(self):
        return self.minimum_bag.power
...
def solve_problem(filename, expected1=None, expected2=None):
    ...
    result2 = sum(game.power for game in games)
    print(f"Part 1: sum of minimum sets power is {result2}")
    if expected2 is not None:
        assert result2 == expected2


def main():
    solve_problem('test.txt', 8, 2286)
    solve_problem('input.txt', 2101, 58269)
